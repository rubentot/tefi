...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const maxFinancing = detectedAmounts && detectedAmounts.length > 0 ? Math.max(...detectedAmounts) : 0;
    ...
    if (maxFinancing === 0) {
      ...
      try { fs.unlinkSync(filePath); fs.rmdirSync(tempDir, { recursive: true }); } catch {}
